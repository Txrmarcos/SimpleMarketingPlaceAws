...
export default function Product() {
    ...
    async function cart(uuid: UUID, email: String) {
        ...
        try {
            const carrinho = await axios.get(`http://0.0.0.0:8081/cart/${uuid}`)
    ...
    async function newWallet(): Promise<any> {
        try {
            ...
            const debito = await axios.get(`http://0.0.0.0:8081/carts`)
    ...
    async function deleteCart(uuid: UUID) {
        try {
            await axios.delete(`http://0.0.0.0:8081/cart/${uuid}`)
    ...
    async function postBuy(productId: number, quant: number, email: String, price: number) {
        ...
        try {
            ...
            await axios.post(`http://0.0.0.0:8081/buy`, {
                productId: productId,
                nItens: quant,
                email: email_,
                price: price
